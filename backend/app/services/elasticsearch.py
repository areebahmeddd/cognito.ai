...
def execute_dsl(es_dsl: Dict[str, Any], index: Optional[str] = None) -> Dict[str, Any]:
    """
    Thin wrapper that executes Elasticsearch DSL exactly as provided by LLM.
    
    This is the new core function that supports the LLM-first NLQ approach.
    It runs the DSL generated by the LLM without any modifications.
    
    Args:
        es_dsl: Complete Elasticsearch DSL body from LLM
        index: Optional index override (defaults to configured index)
    
    Returns:
        Raw Elasticsearch response as dict
        
    Raises:
        Exception: If Elasticsearch execution fails
    """
    target_index = index or index_name
    
    try:
        logger.info(f"Executing LLM-generated DSL on index '{target_index}': {json.dumps(es_dsl, indent=2)[:300]}...")
        
        # Execute the DSL exactly as provided by the LLM
        response = es_client.search(index=target_index, body=es_dsl)
        ...
        total_hits = result_dict.get('hits', {}).get('total', {})
        hit_count = total_hits.get('value', 0) if isinstance(total_hits, dict) else total_hits
        took_ms = result_dict.get('took', 0)
        
        logger.info(f"DSL execution completed: {hit_count} hits in {took_ms}ms")
        
        return result_dict
        
    except Exception as e:
        error_msg = f"Elasticsearch DSL execution failed: {str(e)}"
        logger.error(error_msg)
        logger.error(f"Failed DSL: {json.dumps(es_dsl, indent=2)}")
...
def search_dsl(
    ...
    """Legacy search function - still used by existing code"""
