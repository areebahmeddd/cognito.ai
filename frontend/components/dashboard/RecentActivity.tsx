...
interface ActivityItem {
  id: string;
  type: "case_created" | "search_performed" | "file_uploaded" | "analysis_completed";
  title: string;
  description: string;
  timestamp: string;
  caseId?: string;
...
export default function RecentActivity() {
  ...
  useEffect(() => {
    ...
    if (savedActivities) {
      ...
      localStorage.setItem("cognito-recent-activities", JSON.stringify(mockActivities));
    }
  }, []);
        ...
        {activities.length === 0 ? (
          ...
          activities.map((activity) => (
            <div key={activity.id} className="flex items-start gap-3">
              <div className={`w-10 h-10 ${getActivityColor(activity.type)} rounded-xl flex items-center justify-center flex-shrink-0`}>
